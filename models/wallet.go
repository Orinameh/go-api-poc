...
// GeneratePublicKey function for generating key which is a combination of NickName and Password
...
func GetWallets() ([]Wallet, error) {
	...
	for rs.Next() {
		var wallet Wallet
		err := rs.Scan(&wallet.User.UID, &wallet.User.Nickname, &wallet.User.Email, &wallet.User.Password,
			&wallet.User.Status, &wallet.User.CreatedAt, &wallet.User.UpdatedAt, &wallet.PublicKey,
